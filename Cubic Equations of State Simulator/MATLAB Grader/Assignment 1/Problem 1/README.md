...
EoS Simulator (**Figure 1**) is a program developed in MATLAB<sup>&reg;</sup> R2022a that calculates and plots  diagrams/surfaces of a pure substance. It also compares its results with experimental values ​​or results obtained by Antoine's equation. The requirements to install and run this application are:
...
The application is in a zipped folder located in a reportory on GitHub®. The steps to download and install the simulator are:
